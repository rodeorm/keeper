...
func testStorage(ctx context.Context, srv *cfg.Server) {
	...
	cards, _ := srv.CardStorager.SelectCardByUser(ctx, usr1)
	for i, v := range cards {
		fmt.Println(i, v)
	}
	bins, _ := srv.BinaryStorager.SelectBinaryByUser(ctx, usr1)
	for i, v := range bins {
		fmt.Println(i, string(v.Value), v.Meta)
	}
	couples, _ := srv.CoupleStorager.SelectCoupleByUser(ctx, usr1)
	for i, v := range couples {
		fmt.Println(i, v)
	}
	texts, _ := srv.TextStorager.SelectTextByUser(ctx, usr1)
	for i, v := range texts {
		fmt.Println(i, v)
	}
