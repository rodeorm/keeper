...
func NewSender(id int, queue *Queue, storage core.MessageStorager, smtpPort int, smtpServer, smtpLogin, smtpPassword string) *Sender {
	s := Sender{
		ID:             id,
		queue:          queue,
...
func (s *Sender) Send(ms *core.Message) error {
	...
	if err := s.dialer.DialAndSend(email.gms); err != nil {
		logger.Info("Send", "отправлено сообщение", ms.Text)
		return err
	}
