...
(
	...
	, Name TEXT
	, PRIMARY KEY (ID) --USING INDEX TABLESPACE ts_indexes 
) WITH (OIDS=FALSE, FILLFACTOR=90) -- TABLESPACE ts_data 
;
COMMENT ON TABLE cmn.Roles is 'Роли пользователя';
COMMENT ON COLUMN cmn.Roles.ID is 'Уникальный идентификатор';
COMMENT ON COLUMN cmn.Roles.Name is 'Наименование роли пользователя';
...
COMMENT ON COLUMN cmn.Sessions.LastDate 	is 'Дата и время последнего действия пользователя в Личном кабинете';	
COMMENT ON COLUMN cmn.Sessions.LogoutDate is 'Дата и время выхода пользователя из Личного кабинета';	
CREATE INDEX IF NOT EXISTS idx_sessions_id ON cmn.Sessions (ID) INCLUDE (LoginDate, LastDate, LogoutDate);
